...
class Navigation(Node):
    ...
    def create_marker(self, x, y, id):
        ...
        marker.header.frame_id = "map"  # Make sure this matches RViz's fixed frame
    ...
    def path_follower(self, vehicle_pose, current_goal_pose):
        ...
        # print("head",np.degrees(heading),self.i)
    ...
    def move_ttbot(self, speed, heading):
        ...
        orr_error=desired_heading-current_heading[2]
        pos_error_x=desired_position[0]-self.ttbot_pose.pose.position.x
        pos_error_y=desired_position[1]-self.ttbot_pose.pose.position.y
        distance = np.sqrt(pos_error_x**2 + pos_error_y**2)
    
        # Normalize the heading error to be within -pi to +pi (or -180 to 180 degrees)
        orr_error = (desired_heading - current_heading[2] + np.pi) % (2 * np.pi) - np.pi

        if abs(orr_error) > 0.2: 
            cmd_vel.linear.x = 0.0  
            cmd_vel.angular.z = 1.5 * orr_error 
        else:
        ...
        if abs(orr_error) < 0.2 and distance < 0.02:
            cmd_vel.linear.x = 0.0
            cmd_vel.angular.z = 0.0
            self.cmd_vel_pub.publish(cmd_vel)  # Stop the robot
            print("Reached target position and heading",self.i)
            marker = self.create_marker(desired_position[0], desired_position[1], self.i)
            

            # Check if marker is valid before publishing
            if marker is not None:
                self.marker_pub.publish(marker)  
            else:
                self.get_logger().error("Failed to create a valid marker.")
            self.i+=1
            return False  
    ...
    def run(self,path):
        ...
        while rclpy.ok():
            ...
            # current_goal = path[]
            # print(current_goal)
            print("crrent goal",current_goal)
            while self.a==True:
                rclpy.spin_once(self, timeout_sec=0.001) 
...
#     # plt.show()
    ...
    yaml_file = '/home/ros2/turttle/src/sim_ws/src/task_7/maps/maps/sync_classroom_map.yaml'  # Path to your YAML file
...
#     print(real_path)
    ...
    try:
        ...
        # print("goal",(,)
        mp.map_graph.root = "84,108"
        # mp.map_graph.root = "47,128"
    # mp.map_graph.root = "74,24"
        # mp.map_graph.end = (-goaly + origin[1])/resolution,(goalx - origin[0])/resolution # enter option y,x
        x = (-goaly + origin[1]) / resolution
        y = (goalx - origin[0]) / resolution

        # Convert the tuple to a string "x,y"
